...
export const serialize = <T extends JSONObject | JSONObject[], TRep extends T = T>(
  obj: T,
  rep?: TRep,
) => {
...
>(
  ...
  if (Array.isArray(representation)) {
    return serialized.map((value, i) => {
      return !value || typeof value !== "object" ? value : deserialize(value, representation[0]);
    }) as R;
  }
    ...
    (acc, value, i) => {
      ...
      acc[repKey] =
        !value || typeof value !== "object"
          ? value
          : typeof representation[repKey] === "object" && representation[repKey]
...
export const createRepresentation = <T extends JSONObject>(representative: T): T => {
  ...
  return Object.entries(representative).reduce(
    (acc, [key, value]) => {
