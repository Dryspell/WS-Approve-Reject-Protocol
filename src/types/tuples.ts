...
export type TupleFilterByIndex<T extends any[], N extends number | `${number}`> = TupleExclude<
  { [K in keyof T]: `${K}` extends `${N}` ? T[K] : typeof Sigil },
  typeof Sigil
>;

export type TupleExclude<T extends any[], V, A extends any[] = []> = T extends [infer F, ...infer R]
