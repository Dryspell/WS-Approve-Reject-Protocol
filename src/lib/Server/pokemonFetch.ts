...
export type PokemonFetchHandlerArgs = [
  ...
  callback: (
    returnData:
      | [returnType: SC_ComType.Approve, comId: string, returnData: PokemonApiResponse]
        ...
        case CS_ComType.Get: {
          ...
          axios({
            ...
            if (response.status === 200) {
              callback([SC_ComType.Approve, comId, response.data]);
            } else {
              callback([SC_ComType.Reject, comId, ["Pokemon not found"]]);
            }
          });
